...
{   
    logger.log(INFO_LOG," licenseNumber : "+ licenseNumber); 
    logger.log(INFO_LOG," fronttruckId : "+ truckId); 
    logger.log(INFO_LOG," gateData.licenceNumber : " +frontGateData.licenceNumber );
    logger.log(INFO_LOG," gateData.frontTrackId : "+ frontGateData.frontTrackId); 
    logger.log(INFO_LOG," gateData.truckIdNumber : "+ frontGateData.truckIdNumber); 
    ...
    if (frontGateData.frontConditionCounter >= counterSize && (!licenseNumber.empty() || !truckId.empty()) ) {
        ...
        if((frontGateData.licenceNumber != licenseNumber) || (frontGateData.frontTrackId != truckId))
        {  
...
void TruckOccupationChecker::handleGate(std::string& Timestamp, const std::string& TruckIdNumber, GateData& sideGateData) {
    ...
    logger.log(INFO_LOG," SideTruckIdNumber : "+ TruckIdNumber + " Timestamp : "+ Timestamp); 
    ...
    formattedTimestamp << std::put_time(&timeInfo, "%Y-%m-%d %H:%M:%S.000");
        ...
        // checking 20 times condition when TruckIdNumber is not empty
        if (++sideGateData.conditionCounter >= counterSize && !TruckIdNumber.empty()){
            ...
            // whenever TruckIdNumber will change or get empty this should triggered
...
void TruckOccupationChecker::processFrameData(const std::string& FrameData, const std::string& truckObjectIdNumber, std::string timeStamp) {
    ...
    handleGate(timeStamp, truckObjectIdNumber, gateDataArray[gateNumber]);
}
...
{   // Assuming bboxesArray is an array and truckIndex is valid
    ...
    std::string TruckId="";
    if (!IdObjArray.Empty() && Index < IdObjArray.Size()) {
    // Check if the element at index Index is of type int
        if (IdObjArray[Index].IsInt()) {
            // Convert the integer to a string
            TruckId = std::to_string(IdObjArray[Index].GetInt());
        } else {
            TruckId="";
        }
    } else {
    ...
    if (bboxesArray.IsArray() && !bboxesArray.Empty()) {
        ...
        {
            newTruckBoundingBox = {0.0, 0.0, 0.0, 0.0};
        }
    } else {
        newTruckBoundingBox = {0.0, 0.0, 0.0, 0.0};
    }
...
void TruckOccupationChecker::processContinuousData(const std::string& FrameData) {
    ...
    if (document.HasParseError() || !document.IsObject()) {
        ...
        logger.log(INFO_LOG, "Successfully parsed JSON data.");
    }

    for (const auto& member : document.GetObject()) {
        if (!member.value.IsObject()) {
            logger.log(ERROR_LOG, "Error parsing JSON data. Expected object for camera entry.");
            continue;
        }
        ...
        if ("front" == camType) {
            processFrontCamera(objectClassesArray, objectidsArray, licensePlateArray);
        } else if ("side" == camType) {
            processSideCamera(objectClassesArray, BboxesArray, objectidsArray, timeStamp);
        }

        processGateData(FrameData, objectClassesArray, BboxesArray, objectidsArray, timeStamp, loadStatus);
    }
}

void TruckOccupationChecker::processFrontCamera(const rapidjson::Value& objectClassesArray,
                                                const rapidjson::Value& objectidsArray,
                                                const rapidjson::Value& licensePlateArray) {
    std::string truckId = "";
    std::string numberPlate = "";

    if (!objectClassesArray.Empty() && objectClassesArray.Size() > 0) {
        for (rapidjson::SizeType i = 0; i < objectClassesArray.Size(); ++i) {
            if (objectClassesArray[i].IsString()) {
                if (strcmp(objectClassesArray[i].GetString(), "truck") == 0) {
                    if (!objectidsArray.Empty() && objectidsArray.Size() > i && objectidsArray[i].IsInt()) {
                        truckId = std::to_string(objectidsArray[i].GetInt());
                    }
                } else if (strcmp(objectClassesArray[i].GetString(), "number_plate") == 0) {
                    if (!licensePlateArray.Empty() && licensePlateArray.Size() > 0 && licensePlateArray[0].IsString()) {
                        numberPlate = licensePlateArray[0].GetString();
                    }
                }
            }
        }
    ...
    logger.log(INFO_LOG, "Front Camera - Truck ID: " + truckId + ", Number Plate: " + numberPlate);
    frontCamTruckandLicense(numberPlate, truckId);
}

void TruckOccupationChecker::processSideCamera(const rapidjson::Value& objectClassesArray,
    ...
    if (!objectClassesArray.Empty() && objectClassesArray.Size() > 0) {
        ...
        if (truckCover) {
            for (rapidjson::SizeType i = 0; i < objectClassesArray.Size(); ++i) {
                if (objectClassesArray[i].IsString() && strcmp(objectClassesArray[i].GetString(), "cover") == 0) {
                    processBboxes(FrameData, BboxesArray, objectidsArray, i, timeStamp.GetString());
                }
            }
        }
    } else {
        logger.log(INFO_LOG, "Empty frames for side camera gate_id: " + gate_id);
        processBboxes("", BboxesArray, objectidsArray, 0, timeStamp.GetString());
    }
}

void TruckOccupationChecker::processGateData(const std::string& frameData,
                                             const rapidjson::Value& objectClassesArray,
                                             const rapidjson::Value& BboxesArray,
                                             const rapidjson::Value& objectidsArray,
                                             const rapidjson::Value& timeStamp,
                                             LoadStatusChecker& loadStatus) {
        ...
        try {
            if (!objectClassesArray.Empty() && objectClassesArray.Size() > 0) {
                ...
                for (rapidjson::SizeType k = 0; k < objectClassesArray.Size(); ++k) {
                    ...
                    if ((strcmp(objectClassesArray[k].GetString(), "cover") == 0) && gateDataArray[std::stoi(gate_id) - 0].ForkliftTrackStatus) {
                        gateDataArray[std::stoi(gate_id) - 0].loadStatusFlagTemp =
                            loadStatus.processBboxes(frameData, BboxesArray, k, gateDataArray[std::stoi(gate_id) - 0].ForkliftTrackStatus, tIndex);
                        break;
                    }
                }
            } else {
                rapidjson::SizeType tmp = 0;
                gateDataArray[std::stoi(gate_id) - 0].ForkliftTrackStatus = false;
                gateDataArray[std::stoi(gate_id) - 0].loadStatusFlagTemp =
                    loadStatus.processBboxes(frameData, BboxesArray, tmp, gateDataArray[std::stoi(gate_id) - 0].ForkliftTrackStatus, tIndex);
            }
        } catch (const std::exception& e) {
            logger.log(ERROR_LOG, "Exception caught: " + std::string(e.what()));
            logger.log(ERROR_LOG, "Exception caught: issue related to loading");
        }

        if (gateDataArray[std::stoi(gate_id) - 0].loadStatusFlagTemp == 1 &&
            ...
            meta.event_update(gateDataArray[std::stoi(gate_id) - 0].eventIdRespose,
                              std::to_string(gateDataArray[std::stoi(gate_id) - 0].truckIdRespose), "",
...
void TruckOccupationChecker::processContinuousData(const std::string& FrameData) {
    ...
    for (const auto& member : document.GetObject()) {
        ...
        for (auto& entry : document.GetObject()) {
            ...
            logMessage << " frame data : " << buffer.GetString();
            logger.log(INFO_LOG, logMessage.str());
        ...
        if("front" == camType){
            try {
                logger.log(INFO_LOG, " front camera  : " );
                std::string truckId = "";
                std::string numberPlate = "";
                if (!objectClassesArray.Empty() && 0 < objectClassesArray.Size()) {
                    for(rapidjson::SizeType i = 0; i < objectClassesArray.Size(); ++i) {
                        logger.log(INFO_LOG, " front camera 1 : " );
                        assert(objectClassesArray[i].IsString());
                        if(strcmp(objectClassesArray[i].GetString(), "truck") == 0) {
                ...
                {
                    logger.log(INFO_LOG, "empty frames for front camera gate_id : "+ gate_id );
                    truckId = "";
                    numberPlate = "";
                    frontCamTruckandLicense(numberPlate, truckId);
                }
        ...
        if ("side" == camType) {
            try {
                logger.log(INFO_LOG, " side camera  : " );
                if (!objectClassesArray.Empty() && 0 < objectClassesArray.Size()) {
                    for(rapidjson::SizeType i = 0; i < objectClassesArray.Size(); ++i) {
                        assert(objectClassesArray[i].IsString());
                        if(strcmp(objectClassesArray[i].GetString(), "truck") == 0) {
                            logger.log(INFO_LOG, " side camera 1 : " );
                            processBboxes(FrameData, BboxesArray, objectidsArray, i, timeStamp.GetString());
                            truckCover = false;
                        } 
                        /* else {
        ...
        if(gateDataArray[std::stoi(gate_id) - 0].startLoadFlag)
        {
            ...
            try {
                if (!objectClassesArray.Empty() && 0 < objectClassesArray.Size()) {
                    ...
                    rapidjson::SizeType tmp = 0;
                    gateDataArray[std::stoi(gate_id) - 0].ForkliftTrackStatus = false;
                    gateDataArray[std::stoi(gate_id) - 0].loadStatusFlagTemp = loadStatus.processBboxes(FrameData, BboxesArray, tmp, gateDataArray[std::stoi(gate_id) - 0].ForkliftTrackStatus, tIndex);
                            break;
                    //
                }
            }
...
/*
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i < gateDataArray.size()) {
                ...
                if (elapsedTime > std::chrono::seconds(120)) {
